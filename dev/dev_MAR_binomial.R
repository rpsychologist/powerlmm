...
p <- study_parameters(design = study_design(family = "binomial"),
                      n1 = 11,
                      T_end = 10,
                      n2 = 100,
...
add_MAR_missing <- function(data) {
    ...
    d$eta <- with(d, p$fixed_intercept + subject_intercept + (p$fixed_slope + subject_slope) * time)
    time <- unique(d$time)
    for(i in seq_along(time)) {
        for(j in unique(d$treatment)) {
            if(i == 1) {
                ...
                y <- d$eta[d$time == time[i-1] & d$treatment == j]
                y <- y - mean(y)
                p_miss <- pnorm(-1.5 + -0.2*y + 0.01*time[i] + 0.3 *j *y)
            }
...
# Simulate ----------------------------------------------------------------
f <- sim_formula("y ~ time*treatment + (1 | subject)", data_transform = add_MAR_missing, family = binomial("logit"))
f0 <- sim_formula("y ~ time*treatment + (1 | subject)", family = binomial("logit"))
...
res_obs_diff <- future_lapply(1:100, sim_obs_mean_diff)
