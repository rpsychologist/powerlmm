...
It is also possible to automatically create the `formula` by leaving it blank. Then the `lmer-formula` is created by including parameters that are not specified as `NA` or `NULL` in the model, see `?create_lmer_formula`. N.B., parameters specified as 0 are included in the model.
