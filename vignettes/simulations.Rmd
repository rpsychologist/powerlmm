...
x <- summary(res, para = "time:treatment", type = "fixed")
...
# combine formulas
sim_formula_compare("posttest" = f_PT,
                    "ANCOVA" = f_PT_pre,
                    "LMM" = f_LDA,
                    "LMM_c" = f_LDA_c)
...
print(x, add_cols = c("var_ratio", "effect_size"))
...
d_hline <- data.frame(effect_size = c(0, 0.8), 
                      x = c(0.05, 0.8))
x <- as.data.frame(x) %>%
    mutate(model = factor(model, levels = c("posttest",
                                            "ANCOVA", 
                                            "LMM", 
                                            "LMM_c"))) 
ggplot(x, aes(model, 
...
TODO: also add pretest as covariate
