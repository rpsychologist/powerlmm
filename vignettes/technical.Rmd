...
For small samples, the choice of degrees of freedom will potentially influence the accuracy of the power analysis a lot. In **powerlmm** it is therfore possible to use Satterthwaite's DF approximation in the power analysis. The degrees of freedom of the *t* distribution is thus,
$$ \nu = \frac{2(\textbf{L}^{\top} \bm{\Sigma}_{\bm{\beta}} \textbf{L})^2}{ V(\textbf{L}^{\top}\bm{\Sigma}_{\bm{\beta}}\textbf{L})}$$
and $V(\textbf{L}^{\top} \bm{\Sigma}_{\beta} \textbf{L})$ is approximated using the delta method

$$ V( \textbf{L}^{\top} \bm{\Sigma}_{\bm{\beta}} \textbf{L}) \cong [\Delta_{f( \bm{ \theta}) }( \bm{\theta})]^{\top} \bm{\Sigma}_{\bm{\theta}} [\Delta_{f(\bm{\theta})}(\bm{\theta})] $$

$\Delta_{f(\bm{\theta})}(\bm{\theta})$ is the gradient of $\textbf{L} V(\bm{\beta}) \textbf{L}$ with respect to $\bm{\theta}$, and  $\bm{\Sigma}_{\bm{\theta}}$ is the asymptotic covariance matrix of the random effects, which is approximated as $\bm{\Sigma}_{\bm{\theta}} = 2 \bm{\mathcal{I}}_E^{-1}$, where $\bm{\mathcal{I}}_E$ is the expected information matrix for the variance parameters. The calculation of $\bm{\mathcal{I}}_E$ is described in Equation 25 in @halekoh2014kenward. However, this implementation involves manipulation $V(\textbf{Y})$, i.e. the full variance-covariance matrix including all $N$ observations, and will run out of memory if 
...
$$
 \text{V}(\delta_1^*) = \frac{1+n_1  \text{V}(\textbf{T}) (n_2 \rho_s r_{\tau} + (1-\rho_1)r_{\tau})}{n_1 n_2 n_3  \text{V}(\textbf{T}) },
$$
