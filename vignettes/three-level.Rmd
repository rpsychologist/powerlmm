...
## Model formulation
In standard multilevel notation the model is
$$
\begin{align}
\text{Level 1}& \notag\\\
...
`cohend`            | `slope_diff/(sigma_subject_intercept^2 + sigma_cluster_intercept^2 + sigma_error^2)`
...
## Power 
The `get_power`-method calculates power using the object created with `study_parameters`.
```{r}
get_power(p)
```
...
# Unbalanced designs
It is possible to define different cluster size both within treatment and between treatment groups using the `per_treatment` function. Here's an example of how to have more clusters in the treatment group.
...
# The design effect and type I error rates if clustering is ignored
...
how many times larger the standard errors from the miss-specified model should be. It will
