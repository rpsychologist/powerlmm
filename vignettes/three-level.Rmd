...
`cohend` (fully nested)    | $\gamma_{11}/(\sigma_{u_0}^2 + \sigma_{v_0}^2 + \sigma^2)$  
`cohend` (partially nested)    | $\gamma_{11}/(\sigma_{u_0}^2 + \sigma^2)$  
...
N.B., that partially nested models standardize Cohen's *d* using the control group's pretest SD. Adding more options, and support for unstandardized effects is a high-priority.
...
N.B., in this case cluster sizes are *not* sampled independently for each treatment group, i.e. both groups will have excactly the same cluster imbalance. To sample each group independently, either from the same or from a different distribution, use `per_treatment`.
